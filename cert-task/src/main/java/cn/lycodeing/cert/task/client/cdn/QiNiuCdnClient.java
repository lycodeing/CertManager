...
import cn.lycodeing.cert.task.utils.HttpClientUtil;
...
public class QiNiuCdnClient {
    ...
    public int executeJob(CdnTaskData cdnTaskData, String cert, String key) {
        try {
            ...
            String responseStr = uploadCert(sslRequest, cdnTaskData);
            handleUploadResponse(responseStr);
            log.info("cert.task uploaded successfully ...........");
            String certId = extractCertId(responseStr);
            updateCdnCert(certId, cdnTaskData);
        } catch (Exception e) {
            log.error("Failed to upload cert.task", e);
            return -1;
        }
        return 0;
    }
    ...
    private String uploadCert(SslRequest sslRequest, CdnTaskData postProcessorData) throws IOException {
