package cn.lycodeing.cert.web.mapper;

import cn.lycodeing.cert.web.domain.RequestsPostProcessors;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;

/**
* @author lycodeing
* @description 针对表【cert_requests_post_processors(证书请求与后置处理器关联表)】的数据库操作Mapper
* @createDate 2024-11-12 22:56:07
* @Entity cn.lycodeing.cert.web.domain.RequestsPostProcessors
*/
public interface RequestsPostProcessorsMapper extends BaseMapper<RequestsPostProcessors> {

}




